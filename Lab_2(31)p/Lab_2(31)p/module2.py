...
class NewFileUniversity(NamedTuple):
    name: str
    city: str
...
def out_universities(ul:list)-> None:
    for i in range(len(ul)):
     ...
     print()
def out_acreditation(nful:list)-> None:
    for i in range(len(nful)):
     print("name: ",nful[i].name)
     print("city: ", nful[i].city)
     print("degree_of_acreditation: ", nful[i].degree_of_acreditation)
     print()
...
def acreditation(ul:list)->list:
    ...
    for i in range(len(ul)):
        if(ul[i].degree_of_acreditation==3 or ul[i].degree_of_acreditation==4):
            nfu=NewFileUniversity(ul[i].name,ul[i].city,ul[i].degree_of_acreditation)
            nful.append(nfu)
    return nful
